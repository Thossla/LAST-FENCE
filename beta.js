(() => {
  ...
  const ui = {
    ...
    inventoryGrid: document.querySelector("#inventory-grid"), inventoryDetail: document.querySelector("#inventory-detail"), inventoryTabs: document.querySelector("#inventory-tabs"), avatarPreview: document.querySelector("#avatar-preview"),
    enchant: document.querySelector("#enchant-screen"), enchantGrid: document.querySelector("#enchant-grid"), enchantWeapon: document.querySelector("#enchant-weapon"), enchantInventory: document.querySelector("#enchant-inventory"), lootReveal: document.querySelector("#loot-reveal"), interaction: document.querySelector("#interaction")
  };

  const palette = { ground: "#10251f", groundAlt: "#17332a", grid: "#4c7b67", wood: "#73401f", woodLight: "#b96f35", iron: "#839292", stone: "#344b47", dark: "#071219", green: "#79dca9", orange: "#f48a4c", red: "#ee5e59", blue: "#66beff", cyan: "#78e9ff", violet: "#b399ff", gold: "#f4ca72", white: "#eaf5ef" };
  ...
  const state = {
    ...
    level: 1, xp: 0, xpToLevel: 100, levelDamage: 0, camera: { x: 0, y: 0, ready: false }, equipment: { boots: 0, pants: 0, chest: 0, helmet: 0, artifact: 0, artifact2: 0, artifact3: 0, weapon: 0, manaWeapon: 0 },
    ...
    riftDust: 30, weaponLevel: 1, armorLevel: 1, weaponEnchant: "none", enchantByWeapon: {}, enchantItemId: "starter-sword", allies: [], admin: false, artifactTimer: 1, artifactTimers: [0,0,0], artifactFx: [], sentinel: null, attackFx: 0, attackMode: "weapon", fortified: 0, shieldTimer: 0, titanTimer: 0, dashCount: 0, baseLevel: 1
  };
  ...
  const labels = {weapon:"WAFFE",manaWeapon:"MANA-WAFFE",helmet:"HELM",chest:"BRUSTPLATTE",pants:"HOSE",boots:"SCHUHE",artifact:"ARTEFAKT 1",artifact2:"ARTEFAKT 2",artifact3:"ARTEFAKT 3"};
  const baseSlot = slot => slot.startsWith("artifact") ? "artifact" : slot;
  const gearSlots = Object.fromEntries(Object.keys(labels).map(slot => [slot,{label:labels[slot],options:[catalogue.starters[baseSlot(slot)],...catalogue.items.filter(item=>item.slot===baseSlot(slot))]}]));
  const equipped = slot => gearSlots[slot].options[state.equipment[slot]] || catalogue.starters[baseSlot(slot)];
  ...
  function renderLoadout() {
    ...
    Object.entries(gearSlots).forEach(([key, slot]) => {
      ...
      button.className = "loadout-slot"; button.type = "button"; button.draggable = true; button.dataset.slot = key; button.style.setProperty("--slot-color",option.color);
      const count=slot.options.slice(1).filter(owns).length;
      button.innerHTML = `<img class="gear-icon" src="${window.LF_ART.icon(option)}" alt=""><span><small>${slot.label} · ${count}/6</small><strong>${option.name}</strong><small>${itemLine(option)}</small></span>`; ui.loadoutGrid.appendChild(button);
    });
    const value = stats(); ui.loadoutStats.innerHTML = [["KATALOG",`${catalogue.items.filter(owns).length}/42`],["LEBEN", value.maxHp], ["RÜSTUNG", value.armor], ["SCHADEN", value.damage], ["MANA", value.maxMana], ["ZAUBER",`${value.manaDamage} / ${value.manaCost} MANA`],["KRIT",`${Math.round(value.crit*100)} %`]].map(([label, number]) => `<div><span>${label}</span><b>${number}</b></div>`).join("");
    drawAvatarPreview();
  }

  function openLoadout() { closePanels(); state.loadoutOpen = true; renderLoadout(); renderInventory(); ui.loadout.classList.remove("hidden"); if (state.mode === "menu") ui.start.classList.add("hidden"); }
  ...
  function renderInventory() {
    ...
    const equipped = selected.equip && Object.keys(state.equipment).some(slot=>baseSlot(slot)===selected.equip.slot&&state.equipment[slot]===selected.equip.index);
    ...
    ui.inventoryDetail.style.setProperty("--rarity", selected.color); ui.inventoryDetail.innerHTML = `<img class=\"detail-rune gear-icon\" src=\"${window.LF_ART.icon(selected)}\" alt=\"\"><p class=\"rarity-label\">${selected.rarity}</p><h3>${selected.name}</h3><p>${selected.description}</p><p class=\"item-statline\"><b>WERTE</b><br>${selected.stats}${upgradable ? ` · Upgrade-Stufe ${upgradeLevel}` : ""}</p>${action}${upgrade}`;
  ...
  function closePanels() { state.loadoutOpen = false; state.inventoryOpen = false; state.enchantOpen = false; ui.loadout.classList.add("hidden"); ui.enchant.classList.add("hidden"); }
  function openInventory() { openLoadout(); }
  function closeInventory() { closeLoadout(); }

  function enchantChoices(item) { const id=item?.catalogueId||item?.id||""; if (/pyre|ember/.test(id)) return ["burning","shock","vampiric"]; if (/frost|glacier/.test(id)) return ["frost","shock","vampiric"]; if (/storm|thunder|arc/.test(id)) return ["shock","burning","frost"]; if (/soul|grave|void/.test(id)) return ["vampiric","frost","shock"]; return ["burning","frost","vampiric"]; }
  function renderEnchant() {
    const weapons=state.inventory.filter(item=>item.equip&&["weapon","manaWeapon"].includes(item.equip.slot));
    if(!weapons.some(item=>item.catalogueId===state.enchantItemId)) state.enchantItemId=weapons[0]?.catalogueId;
    const weapon=weapons.find(item=>item.catalogueId===state.enchantItemId), activeId=state.enchantByWeapon[state.enchantItemId]||"none", active=enchantments[activeId];
    ui.enchantInventory.innerHTML=weapons.map(item=>`<button type="button" draggable="true" data-enchant-item="${item.catalogueId}" class="enchant-source-item${item===weapon?" selected":""}"><img class="gear-icon" src="${window.LF_ART.icon(item)}" alt=""><span>${item.name}</span></button>`).join("");
    ui.enchantWeapon.style.setProperty("--enchant",active.color);ui.enchantWeapon.innerHTML=weapon?`<img class="gear-icon" src="${window.LF_ART.icon(weapon)}" alt=""><small>WAFFE IM SCHMIEDE-SLOT</small><h3>${weapon.name}</h3><b>${active.name}</b><p>${active.description}</p><em>${state.souls} SEELEN VERFÜGBAR</em>`:"WAFFE HIERHER ZIEHEN";
    ui.enchantGrid.innerHTML=weapon?enchantChoices(weapon).map(id=>{const enchant=enchantments[id];return `<button class="enchant-option${activeId===id?" active":""}" style="--enchant:${enchant.color}" data-enchant-id="${id}" ${activeId===id?"disabled":""}><i>${enchant.icon}</i><span><b>${enchant.name}</b><small>${enchant.description}</small></span><strong>${activeId===id?"AKTIV":`${enchant.cost} SEELEN`}</strong></button>`;}).join(""):"";
  }
  ...
  function applyEnchant(id) { const item=state.inventory.find(entry=>entry.catalogueId===state.enchantItemId),enchant=enchantments[id]; if (!item||!enchant||!enchantChoices(item).includes(id)||state.enchantByWeapon[item.catalogueId]===id) return; if (state.souls < enchant.cost) { showToast(`ZU WENIG SEELEN — ${enchant.cost} BENÖTIGT`); return; } state.souls -= enchant.cost; state.enchantByWeapon[item.catalogueId]=id;state.weaponEnchant=state.enchantByWeapon[equipped("weapon").id]||"none"; renderEnchant(); renderInventory(); particle(state.hero?.x || world.artifact.x, state.hero?.y || world.artifact.y, enchant.color, 30, 150, .9); showToast(`${enchant.name} AUF ${item.name.toUpperCase()} GEBUNDEN`); }

  function project(x,y){return {x:.82*x-.38*y,y:.22*x+.62*y};}
  function camera() {
    const zoom = clamp(Math.min(innerWidth / 1080, innerHeight / 720), .52, 1.02),focus=project((state.hero||world.artifact).x,(state.hero||world.artifact).y);
    return {zoom,x:state.camera.ready?state.camera.x:focus.x-innerWidth/(2*zoom),y:state.camera.ready?state.camera.y:focus.y-innerHeight/(2*zoom)};
  }
  function updateCamera(dt, snap = false) { const zoom=clamp(Math.min(innerWidth / 1080,innerHeight / 720),.52,1.02),focus=project((state.hero||world.artifact).x,(state.hero||world.artifact).y),x=focus.x-innerWidth/(2*zoom),y=focus.y-innerHeight/(2*zoom);if(snap||!state.camera.ready){state.camera.x=x;state.camera.y=y;state.camera.ready=true;return;}const smoothing=Math.min(1,dt*7);state.camera.x+=(x-state.camera.x)*smoothing;state.camera.y+=(y-state.camera.y)*smoothing;}
  function toScreen(x, y, cam) { const p=project(x,y);return { x: (p.x - cam.x) * cam.zoom, y: (p.y - cam.y) * cam.zoom }; }
  function toWorld(x, y) { const cam=camera(),px=x/cam.zoom+cam.x,py=y/cam.zoom+cam.y,det=.82*.62+.38*.22;return {x:(.62*px+.38*py)/det,y:(-.22*px+.82*py)/det}; }
  ...
  function reset(admin=false) {
    ...
    state.level = 1; state.xp = 0; state.xpToLevel = 100; state.levelDamage = 0; state.mapOpen = false; state.loadoutOpen = false; state.inventoryOpen = false; state.enchantOpen = false; state.lootOpen = false; state.activeCache = null; state.riftDust = 30; state.weaponLevel = 1; state.armorLevel = 1; state.weaponEnchant = "none";state.enchantByWeapon={};state.enchantItemId=catalogue.starters.weapon.id;state.attackMode="weapon";state.artifactTimers=[0,0,0]; seedInventory();
  ...
  function fire() {
    const hero=state.hero;if(state.fireTimer>0)return;const weapon=equipped("weapon"),id=weapon.id,values=stats(),start={x:hero.x+Math.cos(hero.angle)*28,y:hero.y+Math.sin(hero.angle)*28};
    ...
    const enchant=state.enchantByWeapon[id]|| (id==="emberforge-maul"?"burning":id==="glacier-breaker"?"frost":id==="gravecaller-staff"?"vampiric":id==="thunderbolt-rifle"?"shock":"none");
    if(id==="thunderbolt-rifle"){
      for(let i=-1;i<=1;i++){const angle=hero.angle+i*.065;state.bullets.push({...start,vx:Math.cos(angle)*1000,vy:Math.sin(angle)*1000,life:.78,damage:damage*.48,radius:4,color:weapon.color,enchant,weaponId:id,owner:"hero"});}
      state.fireTimer=.35;state.attackFx=.3;particle(start.x,start.y,weapon.color,19,175,.35);return;
    }
    const heavy=/maul|breaker/.test(id),twins=id==="voidfang-twins",staff=id==="gravecaller-staff",range=heavy?115:staff?125:twins?89:104,arc=heavy?1.4:staff?1.25:twins?1.8:1.45;
    let hits=0;for(const enemy of [...state.enemies]){const d=distance(hero,enemy),a=Math.atan2(enemy.y-hero.y,enemy.x-hero.x);if(d>range+enemyKinds[enemy.kind].radius||Math.abs(angleDifference(a,hero.angle))>arc/2)continue;enemy.hp-=damage*(twins?.75:heavy?1.25:1);enemy.hitFlash=.2;applyEnchantHit({enchant,damage},enemy);hits++;particle(enemy.x,enemy.y,weapon.color,heavy?17:8,140,.42);if(enemy.hp<=0)defeatEnemy(enemy);}
    const impact={x:hero.x+Math.cos(hero.angle)*range*.7,y:hero.y+Math.sin(hero.angle)*range*.7};
    if(heavy&&hits)areaDamage(impact,id==="glacier-breaker"?112:95,damage*.38,enchant,weapon.color);
    if(staff&&hits)hero.hp=Math.min(hero.maxHp,hero.hp+hits*5);
    state.artifactFx.push({type:heavy?"slam":twins?"twinSlash":staff?"staffSweep":"slash",x:hero.x,y:hero.y,angle:hero.angle,radius:range,color:weapon.color,life:heavy?.48:.32,maxLife:heavy?.48:.32});
    state.attackFx=heavy?.55:.34;state.fireTimer=heavy?.62:staff?.46:twins?.18:.34;particle(impact.x,impact.y,weapon.color,heavy?22:8,115,.42);
  }
  ...
  function updateArtifact(dt){
    state.artifactTimers=state.artifactTimers.map(value=>Math.max(0,value-dt));
  ...
  function activateArtifact(index){
    if(state.mode!=="playing"||state.mapOpen||state.loadoutOpen||state.enchantOpen||state.lootOpen)return;
    const slot=["artifact","artifact2","artifact3"][index],artifact=equipped(slot),hero=state.hero;
    if(artifact.starter){showToast(`ARTEFAKT ${index+1} IST LEER — LOOTDROPS FINDEN`);return;}
    if(state.artifactTimers[index]>0){showToast(`${artifact.name.toUpperCase()} · ${state.artifactTimers[index].toFixed(1)} S COOLDOWN`);return;}
    if(artifact.id==="necromancer-sigil"){const before=state.allies.length;summonSouls();if(state.allies.length>before)state.artifactTimers[index]=5;return;}
    const target=nearestEnemy(hero,artifact.id==="storm-core"?780:430);
    if(artifact.id==="fencers-sigil"&&target){target.hp-=50;target.hitFlash=.2;energyArc(hero,target,artifact.color);state.artifactTimers[index]=5;}
    else if(artifact.id==="storm-core"&&target){target.hp-=88;target.hitFlash=.2;energyArc({x:target.x-15,y:target.y-280},target,artifact.color,.45);let second=nearestEnemy(target,170,target);if(second){second.hp-=44;second.hitFlash=.2;energyArc(target,second,artifact.color);}state.artifactTimers[index]=9;}
    else if(artifact.id==="sun-core"){areaDamage(hero,225,58,"burning",artifact.color);state.artifactTimers[index]=12;}
    else if(artifact.id==="wind-crystal"&&target){const vortex={x:target.x,y:target.y};for(const enemy of state.enemies){if(distance(enemy,vortex)>200||enemy.kind==="breaker")continue;enemy.x+=(vortex.x-enemy.x)*.3;enemy.y+=(vortex.y-enemy.y)*.3;enemy.hp-=40;enemy.hitFlash=.2;}state.artifactFx.push({type:"vortex",x:vortex.x,y:vortex.y,radius:165,color:artifact.color,life:.85,maxLife:.85});particle(vortex.x,vortex.y,artifact.color,40,175,.85);state.artifactTimers[index]=13;}
    else if(artifact.id==="warden-totem"){state.sentinel={x:hero.x+60,y:hero.y+35,life:15,cooldown:.1};particle(state.sentinel.x,state.sentinel.y,artifact.color,30,125,.65);state.artifactTimers[index]=20;}
    else {showToast("KEIN GEGNER IN REICHWEITE");return;}
    showToast(`${artifact.name.toUpperCase()} AKTIVIERT`);
  }
  ...
  function updateHero(dt) {
    ...
    const worldDx=.62*dx+.38*dy,worldDy=-.22*dx+.82*dy,length=Math.hypot(worldDx,worldDy)||1;
    if (dx || dy) { hero.x += worldDx / length * stats().speed * dt; hero.y += worldDy / length * stats().speed * dt; }
    ...
    if (pointer.down || keys.Space) {if(state.attackMode==="manaWeapon")fireManaWeapon();else fire();}
  }
  ...
  function updateUi() {
    ...
    const weapon=equipped("weapon"),manaWeapon=equipped("manaWeapon"),enchant=enchantments[state.enchantByWeapon[weapon.id]||"none"];document.querySelector("#attackModeLabel").textContent=state.attackMode==="manaWeapon"?"AKTIV · MANA-WAFFE":"AKTIV · PRIMÄRWAFFE";document.querySelector("#activeWeapon").textContent=`${weapon.name.toUpperCase()} · ${state.weaponLevel}`;document.querySelector("#activeEnchant").textContent=enchant.name;document.querySelector("#activeEnchant").style.color=enchant.color;document.querySelector("#activeManaWeapon").textContent=manaWeapon.name.toUpperCase();document.querySelector("#manaCost").textContent=`F WECHSELN · KLICK ${stats().manaCost} MANA`;document.querySelector("#activeArtifact").textContent=["artifact","artifact2","artifact3"].map((slot,i)=>`${i+1} ${equipped(slot).starter?"LEER":equipped(slot).name.toUpperCase()}`).join(" · ");document.querySelector("#artifactStatus").textContent=state.artifactTimers.map((time,i)=>`${i+1}: ${time>0?time.toFixed(1)+" S":"BEREIT"}`).join(" · ");document.querySelector("#catalogueCount").textContent=`KATALOG ${catalogue.items.filter(owns).length} / 42${state.admin?" · ADMIN":""}`;
  }
  ...
  function polygon(points,color,cam,stroke){ctx.beginPath();points.forEach(([x,y],i)=>{const p=toScreen(x,y,cam);if(i)ctx.lineTo(p.x,p.y);else ctx.moveTo(p.x,p.y);});ctx.closePath();if(color){ctx.fillStyle=color;ctx.fill();}if(stroke){ctx.strokeStyle=stroke;ctx.stroke();}}
  function rect(x, y, w, h, color, cam, stroke) { polygon([[x,y],[x+w,y],[x+w,y+h],[x,y+h]],color,cam,stroke); }
  function circle(x, y, radius, color, cam, alpha = 1) { const p = toScreen(x, y, cam); ctx.save(); ctx.globalAlpha = alpha; ctx.fillStyle = color; ctx.beginPath(); ctx.ellipse(p.x,p.y,radius*.9*cam.zoom,radius*.48*cam.zoom,.27,0,Math.PI*2);ctx.fill();ctx.restore(); }
  ...
  function drawBackground(cam) {
    ...
    ctx.save();ctx.strokeStyle="rgba(113,178,143,.075)";ctx.lineWidth=1;for(let x=0;x<=world.width;x+=92){const a=toScreen(x,0,cam),b=toScreen(x,world.height,cam);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();}for(let y=0;y<=world.height;y+=92){const a=toScreen(0,y,cam),b=toScreen(world.width,y,cam);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();}ctx.restore();
  ...
  function drawWorldBoundary(cam){ctx.save();ctx.lineWidth=42*cam.zoom;polygon([[38,38],[world.width-38,38],[world.width-38,world.height-38],[38,world.height-38]],null,cam,"#3f4a51");ctx.lineWidth=4*cam.zoom;ctx.setLineDash([30*cam.zoom,18*cam.zoom]);polygon([[38,38],[world.width-38,38],[world.width-38,world.height-38],[38,world.height-38]],null,cam,"#a77945");ctx.restore();}
  ...
  function drawFence(fence, cam) {
    ...
    const a=vertical?toScreen(fence.x,fence.y-34,cam):toScreen(fence.x-34,fence.y,cam),b=vertical?toScreen(fence.x,fence.y+34,cam):toScreen(fence.x+34,fence.y,cam),height=(29+fence.tier*8)*cam.zoom;ctx.save();ctx.fillStyle=shade(color,.52);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.lineTo(b.x,b.y-height);ctx.lineTo(a.x,a.y-height);ctx.closePath();ctx.fill();ctx.strokeStyle=shade(color,1.35);ctx.lineWidth=5*cam.zoom;ctx.beginPath();ctx.moveTo(a.x,a.y-height);ctx.lineTo(b.x,b.y-height);ctx.stroke();for(const end of [a,b]){ctx.fillStyle=shade(color,.7);ctx.fillRect(end.x-4*cam.zoom,end.y-height-8*cam.zoom,9*cam.zoom,height+8*cam.zoom);ctx.fillStyle=fence.tier>1?palette.blue:palette.gold;ctx.globalAlpha=.65;ctx.fillRect(end.x-2*cam.zoom,end.y-height-10*cam.zoom,5*cam.zoom,5*cam.zoom);}ctx.restore();
  ...
  function drawEnemy(enemy, cam) {
    ...
    ctx.save(); ctx.translate(p.x, p.y);ctx.scale(Math.cos(enemy.angle)<-.15?-1:1,1); ctx.fillStyle = "rgba(0,0,0,.36)"; ctx.beginPath(); ctx.ellipse(0, r * .34, r * 1.05, r * .42, 0, 0, Math.PI * 2); ctx.fill();
    if (enemy.kind === "breaker") {
      ctx.fillStyle="#252d31"; ctx.fillRect(-r*.78,-r*.66,r*1.56,r*1.45); ctx.fillStyle="#151b21";ctx.beginPath();ctx.moveTo(r*.78,-r*.66);ctx.lineTo(r*1.02,-r*.9);ctx.lineTo(r*1.02,r*.6);ctx.lineTo(r*.78,r*.79);ctx.fill();ctx.fillStyle="#596166"; ctx.fillRect(-r*.68,-r*1.08,r*1.36,r*.48); ctx.fillStyle="#171d20"; ctx.fillRect(-r*1.24,-r*.48,r*.42,r*1.22); ctx.fillRect(r*.82,-r*.48,r*.42,r*1.22); ctx.fillStyle=palette.orange; ctx.shadowBlur=20;ctx.shadowColor=palette.orange;ctx.fillRect(-r*.25,-r*.18,r*.5,r*.42);ctx.fillRect(-r*.56,-r*.93,r*1.12,r*.12);ctx.fillStyle="#ffbf63";ctx.fillRect(-r*.27,-r*.85,r*.18,r*.12);ctx.fillRect(r*.09,-r*.85,r*.18,r*.12);
  ...
  function drawHero(hero, cam, targetCtx, previewPoint) {
    const ctx=targetCtx||canvas.getContext("2d"),p=previewPoint||toScreen(hero.x,hero.y,cam),s=cam.zoom,w=equipped(state.attackMode),m=equipped("manaWeapon"),chest=equipped("chest"),helm=equipped("helmet"),pants=equipped("pants"),boots=equipped("boots"),pulse=Math.sin(state.elapsed*4);
    ctx.save();ctx.translate(p.x,p.y);ctx.scale(Math.cos(hero.angle)<-.15?-s:s,s);
    ...
    ctx.fillStyle="#111b24";ctx.fillRect(-19,-17,38,40);ctx.fillStyle=chest.color;ctx.fillRect(-17,-14,34,35);ctx.fillStyle=shade(chest.color,.48);ctx.beginPath();ctx.moveTo(17,-14);ctx.lineTo(25,-20);ctx.lineTo(25,17);ctx.lineTo(17,21);ctx.fill();ctx.fillStyle=shade(chest.color,1.35);ctx.fillRect(-18,-16,36,6);ctx.fillStyle=shade(chest.color,.7);ctx.fillRect(-4,-14,8,34);
    ...
    ctx.fillStyle=helm.color;ctx.fillRect(-15,-32,30,21);ctx.fillStyle=shade(helm.color,.55);ctx.beginPath();ctx.moveTo(15,-32);ctx.lineTo(23,-37);ctx.lineTo(23,-17);ctx.lineTo(15,-11);ctx.fill();ctx.fillStyle=shade(helm.color,1.4);ctx.fillRect(-16,-34,32,6);ctx.fillStyle="#071420";ctx.fillRect(-11,-25,22,7);ctx.fillStyle=helm.color;ctx.fillRect(-4,-28,8,13);
    ...
    ["artifact","artifact2","artifact3"].forEach((slot,index)=>{const item=equipped(slot);if(item.starter)return;const ax=-38-index*7+Math.cos(state.elapsed*2.2+index)*5,ay=-22+index*18+Math.sin(state.elapsed*2.2+index)*6;ctx.save();ctx.shadowColor=item.color;ctx.shadowBlur=23;ctx.fillStyle=item.color;ctx.translate(ax,ay);ctx.rotate(state.elapsed*.8);ctx.fillRect(-6,-6,12,12);ctx.strokeStyle=item.color;ctx.strokeRect(-11,-11,22,22);ctx.restore();});
    ctx.restore();
  }
  function drawAvatarPreview(){const canvas=ui.avatarPreview;if(!canvas)return;const context=canvas.getContext("2d");context.clearRect(0,0,canvas.width,canvas.height);const glow=context.createRadialGradient(150,165,18,150,165,155);glow.addColorStop(0,"#23405d");glow.addColorStop(1,"#0a111a");context.fillStyle=glow;context.fillRect(0,0,canvas.width,canvas.height);context.fillStyle="#263d55";context.beginPath();context.ellipse(150,285,95,29,0,0,Math.PI*2);context.fill();drawHero(state.hero||{x:0,y:0,angle:-.35,invulnerable:0},{zoom:2.4},context,{x:150,y:187});}
  ...
  function drawEffects(cam) {
    ...
    state.bullets.forEach(bullet => { const p=toScreen(bullet.x,bullet.y,cam),tail=toScreen(bullet.x-bullet.vx*.02,bullet.y-bullet.vy*.02,cam),color=bullet.color||palette.blue,r=bullet.radius*cam.zoom,t=state.elapsed;ctx.save();ctx.shadowBlur=bullet.mana?28:12;ctx.shadowColor=color;ctx.strokeStyle=color;ctx.fillStyle=color;ctx.lineWidth=(bullet.mana?5:3)*cam.zoom;ctx.beginPath();ctx.moveTo(tail.x,tail.y);ctx.lineTo(p.x,p.y);ctx.stroke();if(bullet.manaId==="pyre-scepter"){for(let i=0;i<3;i++){ctx.fillStyle=["#ff6934","#ffb345","#fff1aa"][i];ctx.beginPath();ctx.arc(p.x-Math.cos(t*12+i)*r*.25,p.y-Math.sin(t*15+i)*r*.3,r*(1.6-i*.35),0,Math.PI*2);ctx.fill();}}else if(bullet.manaId==="frost-prism"){ctx.translate(p.x,p.y);ctx.rotate(t*7);ctx.beginPath();ctx.moveTo(0,-r*2);ctx.lineTo(r*.85,0);ctx.lineTo(0,r*2);ctx.lineTo(-r*.85,0);ctx.closePath();ctx.fill();ctx.strokeStyle="#e6faff";ctx.stroke();}else if(bullet.manaId==="void-grimoire"){ctx.beginPath();ctx.arc(p.x,p.y,r*1.5,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#ffb5f8";ctx.lineWidth=2;ctx.beginPath();ctx.arc(p.x,p.y,r*2.1,0,Math.PI*2);ctx.stroke();}else if(bullet.manaId==="stormcaller-staff"){ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(tail.x,tail.y);ctx.lineTo(p.x+r*.8,p.y-r*.5);ctx.lineTo(p.x-r*.5,p.y+r*.7);ctx.stroke();}else if(bullet.manaId==="soul-lantern"){ctx.beginPath();ctx.arc(p.x,p.y,r*1.25,0,Math.PI*2);ctx.fill();ctx.fillStyle="#f5eaff";ctx.fillRect(p.x-r*.35,p.y-r*.3,r*.2,r*.2);ctx.fillRect(p.x+r*.15,p.y-r*.3,r*.2,r*.2);}else if(bullet.mana){ctx.beginPath();for(let i=0;i<10;i++){const a=i*Math.PI/5+t*3,rr=i%2?r*1.5:r*.75,x=p.x+Math.cos(a)*rr,y=p.y+Math.sin(a)*rr;if(i)ctx.lineTo(x,y);else ctx.moveTo(x,y);}ctx.closePath();ctx.fill();}ctx.restore(); });
  ...
  function equipItem(item,slot){if(!item?.equip||!gearSlots[slot]||baseSlot(slot)!==item.equip.slot||!owns(item))return false;if(baseSlot(slot)==="artifact")for(const other of ["artifact","artifact2","artifact3"])if(other!==slot&&equipped(other).id===item.catalogueId)state.equipment[other]=0;state.equipment[slot]=item.equip.index;state.weaponEnchant=state.enchantByWeapon[equipped("weapon").id]||"none";applyStats();renderLoadout();renderInventory();showToast(`${item.name.toUpperCase()} AUSGERÜSTET`);return true;}
  function dragItem(event){const button=event.target.closest("[data-item-id]");if(!button)return;event.dataTransfer.setData("text/plain",`item:${button.dataset.itemId}`);event.dataTransfer.effectAllowed="move";}
  function dragSlot(event){const button=event.target.closest("[data-slot]");if(!button)return;event.dataTransfer.setData("text/plain",`slot:${button.dataset.slot}`);event.dataTransfer.effectAllowed="move";}
  function dropOnSlot(event){const button=event.target.closest("[data-slot]");if(!button)return;event.preventDefault();const payload=event.dataTransfer.getData("text/plain");if(payload.startsWith("item:")){const item=state.inventory.find(entry=>String(entry.id)===payload.slice(5));if(!equipItem(item,button.dataset.slot))showToast("DIESES ITEM PASST NICHT IN DEN SLOT");}else if(payload.startsWith("slot:")){const from=payload.slice(5),to=button.dataset.slot;if(baseSlot(from)!==baseSlot(to))return;[state.equipment[from],state.equipment[to]]=[state.equipment[to],state.equipment[from]];renderLoadout();renderInventory();}}
  ...
  ui.loadoutGrid.addEventListener("click", event => {const button=event.target.closest("[data-slot]");if(!button)return;const slot=button.dataset.slot,options=gearSlots[slot].options;let next=state.equipment[slot];do{next=(next+1)%options.length;}while(next!==state.equipment[slot]&&!owns(options[next]));if(next===state.equipment[slot]){showToast("NOCH KEINE WEITEREN TEILE GEFUNDEN");return;}equipItem(state.inventory.find(item=>item.catalogueId===options[next].id),slot);});
  ui.loadoutGrid.addEventListener("dragstart",dragSlot);ui.loadoutGrid.addEventListener("dragover",event=>{if(event.target.closest("[data-slot]"))event.preventDefault();});ui.loadoutGrid.addEventListener("drop",dropOnSlot);
  document.querySelector("#inventory").addEventListener("click", openInventory);
  ...
  ui.inventoryGrid.addEventListener("click", event => { const button = event.target.closest("[data-item-id]"); if (!button) return; state.selectedInventory = state.inventory.find(item=>String(item.id)===button.dataset.itemId)?.id||0; renderInventory(); });ui.inventoryGrid.addEventListener("dragstart",dragItem);ui.inventoryGrid.addEventListener("dragover",event=>event.preventDefault());ui.inventoryGrid.addEventListener("drop",event=>{event.preventDefault();const payload=event.dataTransfer.getData("text/plain");if(!payload.startsWith("slot:"))return;const slot=payload.slice(5);if(!gearSlots[slot])return;state.equipment[slot]=0;applyStats();renderLoadout();renderInventory();showToast(`${labels[slot]} ABGELEGT`);});
  ui.inventoryDetail.addEventListener("click", event => { const equip = event.target.closest("[data-equip-slot]"), upgrade = event.target.closest("[data-soul-upgrade]"); if (equip) {const item=state.inventory.find(entry=>entry.equip?.slot===equip.dataset.equipSlot&&entry.equip.index===Number(equip.dataset.equipIndex));if(item)equipItem(item,item.equip.slot);} else if (upgrade) soulUpgrade(upgrade.dataset.soulUpgrade); });
  document.querySelector("#enchant").addEventListener("click", openEnchant); document.querySelector("#close-enchant").addEventListener("click", closeEnchant); document.querySelector("#enchant-back").addEventListener("click", closeEnchant); ui.enchantGrid.addEventListener("click", event => { const button = event.target.closest("[data-enchant-id]"); if (button) applyEnchant(button.dataset.enchantId); });ui.enchantInventory.addEventListener("click",event=>{const button=event.target.closest("[data-enchant-item]");if(button){state.enchantItemId=button.dataset.enchantItem;renderEnchant();}});ui.enchantInventory.addEventListener("dragstart",event=>{const button=event.target.closest("[data-enchant-item]");if(button)event.dataTransfer.setData("text/plain",`enchant:${button.dataset.enchantItem}`);});ui.enchantWeapon.addEventListener("dragover",event=>event.preventDefault());ui.enchantWeapon.addEventListener("drop",event=>{event.preventDefault();const payload=event.dataTransfer.getData("text/plain");if(payload.startsWith("enchant:")&&state.inventory.some(item=>item.catalogueId===payload.slice(8))){state.enchantItemId=payload.slice(8);renderEnchant();}});document.querySelector("#loot-continue").addEventListener("click", closeLootReveal);
  ...
  window.addEventListener("keydown", event => {
    ...
    const key = event.key.toLowerCase(); if (key === "q") dash(); if (key === "r") repairFence(); if (key === "u") upgradeFence(); if (key === "b") expandBase(); if (key === "f"&&state.mode==="playing"){state.attackMode=state.attackMode==="weapon"?"manaWeapon":"weapon";showToast(state.attackMode==="weapon"?"PRIMÄRWAFFE AKTIV":"MANA-WAFFE AKTIV");updateUi();}if (key === "e") interactCache(); if (key === "g"){const index=["artifact","artifact2","artifact3"].findIndex(slot=>equipped(slot).id==="necromancer-sigil");if(index>=0)activateArtifact(index);else summonSouls();}if(["1","2","3"].includes(key))activateArtifact(Number(key)-1);
    ...
    if ((key === "c"||key==="i") && state.mode === "playing" && !state.mapOpen && !state.enchantOpen) { if (state.loadoutOpen) closeLoadout(); else openLoadout(); }
